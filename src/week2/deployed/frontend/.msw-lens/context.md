...
- **server-error** *(500)* — Tests the failure state when the backend cannot load the learning list.
- **timeout** *(delay: infinite)* — Tests the loading state when the request never resolves.

sourceHints:
- `src/app/areas/parking-lot/pages/list.ts`
- `src/app/areas/parking-lot/types.ts`
